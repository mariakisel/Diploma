...
%   $Revision: 1.23.4.4 $  $Date: 2004/10/20 17:54:21 $
...
if ischar(arg1) % execute a callback
    iptcheckstrs(arg1,...
                 {'pixvalmotionfcn','buttondownonimage',...
  ...
  if ~ishandle(arg1) 
        ...
        figureHandle = ancestor(images(1), 'figure');
        ...
        if nargin>1   % pixval(FIG, OPTION)
            iptcheckstrs(arg2,{'on','off'},mfilename,'OPTION',2);
...
axHandles = ancestor(images,'axes');
if iscell(axHandles)
  axHandles = [axHandles{:}];
end
...
if imageHandle ~= 0
   ...
   case 1
      imageType = 'indexed';
      if ~isequal(floor(img),img) || any(~isfinite(img(:)))
        msgId = 'Images:pixval:invalidIndexedImage';
        msg = 'Invalid indexed image.';
        error(msgId,'%s',msg);
      end
   case 2     %Grayscale in standard range
      imageType = 'intensity'; 
   case 3     %Grayscale in nonstandard range
      imageType = 'intensity';
   case 4
      imageType = 'rgb';
   case 5
...
imageHandle = gcbo;
figureHandle = ancestor(imageHandle,'figure');
displayBar = findobj(figureHandle, 'Tag', 'pixel value display bar');
dbud = get(displayBar, 'UserData');
