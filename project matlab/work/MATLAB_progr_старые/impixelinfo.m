...
%   $Revision: 1.1.8.1 $  $Date: 2004/08/10 01:49:12 $
...
% Make sure impixelinfo can update itself if an image has been added to
% hFig.
...
reactToImageChangesInFig(imageHandles,@deleteFcn);
    ...
    function hPixInfoPanel = createPanel
        ...
        hPixelInfoValue =impixelinfoval(hPixInfoPanel,imageHandles);             %ZZZубрано мной
              %%ZZ hPixelInfoValue = impixelinfoval(hPixInfoPanel,imageHandles); %ZZZдобавлено мной
...
if isequal(parent,ancestor(imageHandles,'figure')) && strcmp(get(parent, ...
                                                    'Visible'),'on')
    figure(parent);
end
